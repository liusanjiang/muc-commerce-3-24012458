...
def metrics_api():
    ...
    # 确保每条记录含有 label、value、note 字段
    return jsonify({"ok": True, "metrics": metrics_data})
...
def unauthorized(_error):
    """401 错误：未认证"""
    return jsonify({"ok": False, "error": "请先登录后再访问。"}), 401


@app.errorhandler(404)
def page_not_found(_error):
    """404 错误：资源不存在"""
    # 如果前端是 SPA / Ajax 请求，统一返回 JSON；否则可保留 render_template
