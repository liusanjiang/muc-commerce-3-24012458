...
def load_dashboard_data(base_dir: Path, selected_category: str = "全部") -> dict:
    ...
    return {
        ...
        "category_rows": table_df.to_dict("records"),  # ✅ 已转为普通Python列表（字典组成）
        "insight": insight,
    }
...
def load_metric_api_data(base_dir: Path) -> list[dict]:
    ...
    return data["metrics"]  # metrics 本身就是 [{"label":..., "value":..., "note":...}, ...]，已是普通Python结构
...
def load_category_api_data(base_dir: Path, selected_category: str = "全部") -> list[dict]:
    ...
    return data["category_rows"]  # category_rows 是 table_df.to_dict("records")，已是普通Python列表（字典组成）